...
@section('content')
                        ...
                        <div class="col-md-8">
                            <div class="form-group mb-0">
                                ...
                                <div class="btn-group" role="group" style="display: inline-flex;">
                                    ...
                                    <button type="button" class="btn btn-outline-success filter-btn" data-filter="large"
                                        title="Cổ đông có tỷ lệ cổ phần từ 5% trở lên">
                                        <i class="fas fa-chart-line"></i> Cổ đông lớn (≥5%)
                                    </button>
                                    <button type="button" class="btn btn-outline-warning filter-btn" data-filter="small"
                                        title="Cổ đông có tỷ lệ cổ phần dưới 5%">
                                        <i class="fas fa-chart-pie"></i> Cổ đông nhỏ (<5%) </button>
                                </div>
                            </div>
                        </div>
                        {{-- <div class="col-md-4">
    ...
    <div class="modal fade" id="importPreviewModal" tabindex="-1" role="dialog"
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var table = $('#securities-table').DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: 'group1_personal',
                        name: 'group1_personal',
                        orderable: false,
                        searchable: false
                    },
                    {
                        data: 'group2_investor',
                        name: 'group2_investor',
                        orderable: false,
                        searchable: false
                    },
                    {
                        data: 'group3_deposited',
                        name: 'group3_deposited',
                        orderable: false,
                        searchable: false
                    },
                    {
                    ...
                    {
                        data: 'group5_classification',
                        name: 'group5_classification',
                        orderable: false,
                        searchable: false
                    },
                    {
            ...
            $('.filter-btn').click(function() {
                $('.filter-btn').removeClass('active').addClass('btn-outline-primary').removeClass(
                    'btn-primary');
            ...
            $('#confirmImport').click(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#confirmImport').prop('disabled', false).html(
                            '<i class="fas fa-check"></i> Xác nhận import');

                        toastr.success(response.message + ' (' + response.processedRows +
                            ' dòng)');
        ...
        function showImportPreview(response) {
            ...
            if (insertCount === 0 && updateCount === 0) {
                html =
                    '<div class="alert alert-info"><i class="fas fa-info-circle"></i> <strong>Không có dữ liệu thay đổi.</strong> File này không chứa bất kỳ nhà đầu tư mới nào hoặc cập nhật nào so với dữ liệu hiện tại.</div>';
            } else if (preview.length === 0) {
                html =
                    '<div class="alert alert-warning"><i class="fas fa-exclamation-triangle"></i> <strong>Cảnh báo!</strong> Không thể phân tích được dữ liệu từ file. Vui lòng kiểm tra lại định dạng file.</div>';
            } else {
                        ...
                        if (item.changes && Object.keys(item.changes).length > 0) {
                            $.each(item.changes, function(field, change) {
                                ...
                                html += '<div class="change-item">' +
                                    ...
                                    '<span class="change-old"><i class="fas fa-times-circle"></i> Cũ: ' + (
                                        change.old || '(trống)') + '</span><br>' +
                                    '<span class="change-new"><i class="fas fa-check-circle"></i> Mới: ' + (
                                        change.new || '(trống)') + '</span>' +
                                    '</div>';
                            });
                        } else {
        ...
        $('#confirmDelete').click(function() {
            if (deleteId) {
                $.ajax({
                    url: "{{ route('admin.securities.management.destroy', ':id') }}".replace(':id',
                        deleteId),
                    type: 'DELETE',
                    data: {
